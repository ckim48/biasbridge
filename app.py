...
@app.route('/submit-alignment/<phase>/<int:debate_id>', methods=['POST'])
def submit__alignment():
    sum = 0
    choice = 1
    if choice == 1:
        sum += 1
...
NEWS_API_KEY = 'cafbae1da4744c3c907cfb0a51566f10'

def get_related_news(topic):
    try:
        url = f"https://newsapi.org/v2/everything?q={topic}&sortBy=publishedAt&language=en&pageSize=15&apiKey={NEWS_API_KEY}"
...
def debate(debate_id):
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        news_articles = get_related_news(topic)
